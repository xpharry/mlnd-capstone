...
Coaster Racer Flash Game is a typical car racing game wrapped in OpenAI-Universe which is simply controlled by moving forward, left turn and right turn. With this simple setting, we only only an action space of three actions, namely, moving forward, left turn and right turn. The screen view as shown in Figure 2 is for the Gamebot. After preprocessing, the image as the bot see would be thrown into a Deep Convolutional Network and output an expected action. The Q function would compute a reward based on the current action. Through a long time learning or training, it would be expected that the Gamebot would grasp a reasonable driving behavior closing to human level.

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{coaster-racer}
\caption{A screenshot of Coaster Racer Game.}
\end{figure}
